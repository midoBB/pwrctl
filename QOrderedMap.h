...
public:
  ...
  public:
    ...
    const_iterator(const iterator &i) : qllConstIter(i.qllIter), data(i.data) {}
...
OrderedMap<Key, Value>::OrderedMap(
  ...
  typedef typename std::initializer_list<std::pair<Key, Value>>::const_iterator
      const_initlist_iter;
...
QList<Key> OrderedMap<Key, Value>::keys() const {
  QList<Key> list;
  for (const auto &key : insertOrder) {
    list.append(key);
  }
  return list;
}
...
QList<Value> OrderedMap<Key, Value>::values() const {
  QList<Value> values;
  foreach (const Key &key, insertOrder) {
...
template <typename Key, typename Value>
bool OrderedMap<Key, Value>::operator==(
    const OrderedMap<Key, Value> &other) const {
